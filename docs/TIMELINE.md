...
- **Representative accountability → community self-enrollment** — "claim your ward" (one per ward, GPS-detected, community-flaggable). Then **de-politicized into a universal civic accountability platform**: reps enrol with a neutral **civic role** (corporator / RWA / volunteer / officer / NGO / independent — `CIVIC_ROLES`); political party is an **optional, muted, never-ranked** field; the "By Party" comparison became a neutral **By-Role** aggregate (`aggregateByRole`). Seeded demo data reseeded role-based (no party labels).
- **Civic collaboration layer ("GitHub for civic issues")** — additive (representatives/authority/ESG untouched): any citizen **Joins** an issue as a civic role, posts **evidence** (base64, Gemini-Vision relevance-gated) + **updates** to an immutable **activity timeline** (`issues/{id}/timeline` & `/evidence` subcollections), and the community **verifies** the fix (2 km live-GPS + 24h-since-join gates → threshold flip). Earned **Community Reputation** (extends `civicScore`) + new badges; lead moderation (open/close, remove); claim-on-view close reward. The **resolution predictor** became collaboration-aware + re-runnable, and the duplicate detector now spans `Needs Verification`. Reconciled to the free plan — base64 + subcollections (no Cloud Storage), Lucide, Gemini Flash.
- **Hardening / bug fixes** — Firestore `experimentalAutoDetectLongPolling` (profile edits failed on some Android networks); Authority Dashboard `canAct = authorized && qualified` (authority mode no longer flashes before the civic-score check); Profile "Amplify reach" + Journalist filter chips realigned; collaboration gaps closed (evidence gallery, post-update UI, reachable Top Verifier badge).
- **Step 3 UX** — Leaderboard **time filters** (All Time / This Month / This Week — a windowed "most active" ranking derived from issues reported in the window), and **collaboration notifications** (contributors joined your report; your report needs community verification) added to the client-side notification feed.
- **Two-sided notifications (contributor-side)** — the feed was reporter-only; extended `useNotifications` so anyone who **joins** an issue is now notified about it too: a joined issue **resolved** (→ open to claim the **+25**), its **status changed**, it **needs their verification vote**, **evidence/updates from other contributors**, or they were **removed**. Purely client-derived (no backend) — a second `useIssues` query on `contributedUids` (new composite index) + per-issue timeline reads, fed into a new pure `utils/notifications.js` (`buildNotifications`) shared by both the bell badge and `/notifications`. New `notifications.test.js` (suite → **23 files / 158 tests**).
- **iOS install fix** — the "Add to Home Screen" banner was inert text (iOS Safari can't be prompted programmatically); rebuilt `InstallBanner` so the iOS banner is **tappable → a step-by-step guide** (Share → Add to Home Screen), and non-Safari iOS browsers are told to open in Safari first.
- **Agent 7 — autonomous Resolution Coordinator** — the agentic-depth upgrade: `agents/resolutionCoordinator.js` (`coordinateResolution`) runs a bounded **ReAct loop** on a stalled/aged/recurring issue. Each turn Gemini **function-calling** returns one `{ action, reasoning }`; the agent executes a real tool — **escalate** (`checkAndEscalate`), **draft RTI** (`generateRTI`), **re-route** (`routeToAuthority`), or **request verification** (`markNeedsVerification`) — observes the result, and decides again, self-correcting (e.g. pivots when escalation is maxed). Capped at 4 turns with no-repeat guardrails; the reasoning streams live via `CoordinatorPanel.jsx` (owner on `IssueDetail`, authority on `AuthorityDashboard`). Persists `coordination` on the issue + an `agent_runs` trace; `authorityFields()` gained `coordination` + `communityVerification`. New deterministic test `resolutionCoordinator.test.js` (suite now **22 files / 146 tests**). This is the first agent that *plans and acts* rather than running on rails.
